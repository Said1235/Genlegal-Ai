...
function AnalyzeForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { receipt, analysisId } = await submit(title.trim(), codeLanguage, text.trim());

      // analysisId is decoded straight from THIS transaction's own return
      // value (see SmartContractAuditor.extractReturnedId) - not guessed by
      // diffing lists, which could return a different transaction's record
      // if the same wallet had another submission land in between. If the
      // decode genuinely failed, fail honestly instead of picking a record
      // that might not be this one.
      if (!analysisId) {
        throw new Error(
          "The analysis was accepted on-chain, but its id could not be read from the transaction receipt - check the Documents tab for it (it will show up shortly)."
        );
      }

      setStatusText("Reading the audit result…");
      const [newAnalysis, contractHash] = await Promise.all([
        contract!.getAnalysis(analysisId),
        sha256Hex(text.trim()),
      ]);
