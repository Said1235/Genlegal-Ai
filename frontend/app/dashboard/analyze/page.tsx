...
import {
  ...
  Code2, ChevronDown, ChevronUp, FolderOpen,
} from "lucide-react";
import { CodeEditor } from "@/components/dashboard/CodeEditor";
import { WalletGate } from "@/components/dashboard/WalletGate";
import { useAnalyzeContract, useContract } from "@/lib/hooks/useSmartContractAuditor";
import { useWallet } from "@/lib/genlayer/WalletProvider";
import { useToast } from "@/lib/toast";
import { useNotificationSettings } from "@/lib/notifications";
import { useLanguage } from "@/lib/i18n";
import { GENLAYER_CHAIN } from "@/lib/genlayer/client";
import { explorerTxUrl, explorerContractUrl, MAX_CONTRACT_TEXT_CHARS } from "@/lib/constants";
...
const LANGUAGES = ["Solidity", "Vyper", "GenLayer (Python)", "Rust", "Move", "Other"];
// Aligned with MAX_CODE_CHARS in contracts/smart_contract_auditor.py -
// the contract rejects anything longer, so the form must match it exactly.
const MAX_CHARS = MAX_CONTRACT_TEXT_CHARS;
const MIN_CHARS = 20;
...
interface ResultData {
  ...
  txHash: string;
  contractHash: string;
  contractText: string;   // ← kept so user can review what was analyzed
}
...
export default function AnalyzeContractPage() {
  ...
  if (view === "analyzing") return <AnalyzingScreen statusText={statusText} />;
  ...
  return (
    <div className="space-y-6">
      ...
      <AnalyzeForm
        address={address!}
        onAnalyzing={(s) => { setStatusText(s); setView("analyzing"); }}
...
function AnalyzeForm({
  ...
  const [codeLanguage, setCodeLanguage] = useState(LANGUAGES[0]);
  ...
  const handleSubmit = async () => {
    ...
    if (text.trim().length > MAX_CHARS) {
      setValidationError(`This contract is too long (${text.trim().length.toLocaleString()} characters). The maximum is ${MAX_CHARS.toLocaleString()}.`);
      return;
    }
    ...
    try {
      ...
      // Snapshot this wallet's own analysis ids *before* submitting. The
      // contract's analyze_contract() returns the new id, but GenLayer write
      // calls only surface a transaction hash to the client - not the
      // method's return value - so the id has to be identified afterwards.
      // Diffing against this wallet's own list (not the global list) means
      // another user's transaction landing in between can never be mistaken
      // for this one.
      const before = await contract!.getMyAnalyses(address);
      const beforeIds = new Set(before.map((a) => a.id));

      const receipt = await submit(title.trim(), codeLanguage, text.trim());

      setStatusText("AI is analyzing your code…");
      const [afterMine, contractHash] = await Promise.all([
        contract!.getMyAnalyses(address),
        sha256Hex(text.trim()),
      ]);
      const created = afterMine.find((a) => !beforeIds.has(a.id));
      if (!created) {
        throw new Error(
          "The analysis was accepted on-chain but could not be located automatically - check the Documents tab."
        );
      }
      // Read the exact record straight from the contract instead of trusting
      // the array entry from the list call above.
      const newAnalysis = await contract!.getAnalysis(created.id);

      rememberTxHash(newAnalysis.id, receipt.hash);
      storeContractText(newAnalysis.id, text.trim()); // ← available in Documents/History
      if (isEnabled("analysisCompleted")) showToast("Analysis complete – stored on GenLayer.", "success");
      onResult({ analysis: newAnalysis, txHash: receipt.hash, contractHash, contractText: text.trim() });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium text-white/50">Language</label>
          <select
            value={codeLanguage}
      ...
      <div>
        ...
        <CodeEditor value={text} onChange={setText} placeholder="Paste your smart contract source code here..." minHeight={320} />
      </div>
...
function AnalyzingScreen({ statusText }: { statusText: string }) {
  ...
  useEffect(() => {
    const id = setInterval(() => setStepIdx((i) => Math.min(i + 1, ANALYSIS_STEPS.length - 1)), 2200);
...
function ResultsScreen({
  ...
  const handleOpenExplorer = () => {
    const url = txHash ? explorerTxUrl(txHash) : explorerContractUrl();
    window.open(url, "_blank", "noreferrer");
  };

  return (
    <div className="space-y-5">
      {/* ── Hero result: Risk Score + Summary ── */}
      <div className="rounded-2xl border border-white/10 bg-bg-card/60 p-5 sm:p-6">
        <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-start">
          <div className="flex shrink-0 flex-col items-center gap-2">
            <div className="relative flex h-28 w-28 items-center justify-center">
              <svg width={112} height={112} className="-rotate-90">
                ...
                <circle cx={56} cy={56} r={48} stroke={riskColor[analysis.risk_level]} strokeWidth={8} fill="none"
                  strokeLinecap="round"
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="mt-0.5 text-xs text-white/40">{analysis.language} · Analyzed {new Date(analysis.created_at).toLocaleString()}</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {/* Explorer: copies txHash + opens GenLayer Studio main page */}
              <ActionBtn icon={ExternalLink} label="Open Explorer" onClick={handleOpenExplorer} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/8 bg-bg-card/60 p-5">
          ...
          {analysis.recommendations.length === 0 ? (
            <p className="text-sm text-white/40">No specific recommendations extracted.</p>
          ) : (
            <ul className="space-y-2">
              {analysis.recommendations.map((o, i) => (
      ...
      {/* ── Collapsible contract code viewer ── */}
      <ContractCodeViewer text={contractText} open={codeOpen} onToggle={() => setCodeOpen((o) => !o)} />

      {/* ── Quick access buttons ── */}
      <div className="grid gap-3 sm:grid-cols-2">
        <button
          onClick={onNewAnalysis}
          className="group relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-accent-gradient py-3.5 text-sm font-semibold text-white shadow-glow transition hover:opacity-90"
        >
          <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-500 group-hover:translate-x-full" />
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 bg-bg-card/60 py-3.5 text-sm font-semibold text-white/80 transition hover:border-accent/40 hover:bg-accent/5 hover:text-white"
        >
...
function ContractCodeViewer({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/8 bg-[#08080f]">
      {/* Header — always visible, triggers expand/collapse */}
      <button
        onClick={onToggle}
      ...
      >
        ...
        <span className="flex items-center gap-2 text-xs text-white/40">
          {open ? (
            <>Hide contract <ChevronUp className="h-4 w-4" /></>
          ) : (
            <>View contract <ChevronDown className="h-4 w-4" /></>
          )}
        </span>
      </button>
      ...
      {open && (
        <>
          ...
          <div className="flex max-h-[480px] overflow-y-auto font-mono text-[12px] leading-[1.7rem] scrollbar-thin">
            {/* Line numbers */}
            <div className="select-none border-r border-white/5 bg-white/[0.01] px-3 py-4 text-right text-accent-light/40">
              {lines.map((_, i) => (
                <div key={i}>{i + 1}</div>
              ))}
            </div>
            {/* Code content */}
            <div className="flex-1 overflow-x-auto whitespace-pre px-4 py-4 text-white/75">
              {lines.map((line, i) => (
                <div key={i}>{line || " "}</div>
              ))}
            </div>
...
function InfoRow({ icon: Icon, label, value, mono, valueClass }: {
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={cx("truncate text-right", mono ? "font-mono text-[12px]" : "", valueClass || "text-white/80")}>
        {value}
      </span>
    </div>
  );
