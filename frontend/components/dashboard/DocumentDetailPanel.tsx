...
type TabKey = "summary" | "recommendations" | "risks" | "code" | "metadata";
...
export function DocumentDetailPanel({
  ...
  const handleCopyCode = async () => {
    ...
    showToast("Contract code copied.", "success");
  };
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-white/40">
              #{analysis.id} · {analysis.language} · Analyzed {new Date(analysis.created_at).toLocaleString()}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-[auto_1fr] sm:p-5">
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            <RiskCounter label="High Sev."      count={analysis.risks.filter(r => /high/i.test(r)).length || 0}   color="text-rose-400"   />
            <RiskCounter label="Medium Sev."    count={analysis.risks.filter(r => /medium/i.test(r)).length || 0} color="text-amber-400"  />
            <RiskCounter label="Recommendations" count={analysis.recommendations.length}                                    color="text-accent-light" />
          </div>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5">
        ...
        {tab === "code" && (
          contractText ? (
            ...
            <div className="flex flex-col items-center gap-2 py-8 text-center text-white/35">
              ...
              <p className="text-sm">Contract code not available for this analysis.</p>
        ...
        {tab === "metadata" && (
          <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            ...
            <MetaRow label="Language"       value={analysis.language} />
