...
export function useMyAnalyses(ownerAddress: string | null) {
  ...
  const refetch = useCallback(async () => {
    if (!contract || !ownerAddress) { setAnalyses([]); return; }
    setLoading(true);
    try {
...
/** Returns the EXACT analysis created by this transaction via ID prediction. */
export function useAnalyzeContract() {
  ...
  const submit = useCallback(
    async (title: string, contractType: string, text: string) => {
      ...
      try {
        return await contract.analyzeContractAndRead(title, contractType, text);
      } catch (err: any) {
