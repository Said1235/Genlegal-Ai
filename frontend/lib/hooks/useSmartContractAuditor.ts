...
import SmartContractAuditor from "../contracts/SmartContractAuditor";
...
export function useContract(): SmartContractAuditor | null {
  const { address } = useWallet();
  const contractAddress = getContractAddress();

  return useMemo(() => {
    if (!contractAddress) return null;
    return new SmartContractAuditor(contractAddress, address);
...
export function useAnalyzeContract() {
  ...
  const submit = useCallback(
    async (title: string, language: string, code: string) => {
      if (!contract) throw new Error("Contract not configured - set NEXT_PUBLIC_CONTRACT_ADDRESS.");
      if (!address) throw new Error("Connect your wallet first.");
      setIsSubmitting(true);
      setError(null);
      try {
        const result = await contract.analyzeContract(title, language, code);
        return result; // { receipt, analysisId }
      } catch (err: any) {
