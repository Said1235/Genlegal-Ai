...
same as an independent on-chain read. Likewise, `extractReturnedId`'s decode
path was verified by reading `genlayer-js@1.1.8`'s own source (see
"Identifying the exact analysis a transaction created" above) and confirmed
to compile (`tsc`, `next build`), but could not be exercised against a real
`analyze_contract` transaction end-to-end for the same network-access
reason - test it against a real submission before relying on it in
production.
