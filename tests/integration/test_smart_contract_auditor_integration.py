"""
Integration tests for SmartContractAuditor.
...
CONTRACT_PATH = Path(__file__).parent.parent.parent / "contracts" / "smart_contract_auditor.py"


def _load_prompt_builder():
    """Import _build_audit_prompt straight from the contract file so the
    mocked-response prompt key always matches the real prompt byte-for-byte,
    even if the wording in the contract changes later."""
    spec = importlib.util.spec_from_file_location("smart_contract_auditor", CONTRACT_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)  # type: ignore[union-attr]
    return module._build_audit_prompt


# Note: risk_level is intentionally absent - the contract derives it from
# risk_score deterministically (see _derive_risk_level), never trusting the
# LLM's own label. 48 -> "Medium" (34-66).
VALID_RESPONSE = {
    "summary": "A simple ERC-20-style token contract with mint, burn, and transfer functions.",
    "recommendations": ["Add a max-supply cap to the mint function"],
    "risks": ["No cooldown or rate limit on minting"],
    "risk_score": 48,
}

SAMPLE_CODE = "contract Token { function mint(address to, uint amount) public { } }" * 5
SAMPLE_LANGUAGE = "Solidity"
...
def mock_validators():
    ...
    prompt = build_prompt(SAMPLE_LANGUAGE, SAMPLE_CODE.strip())
...
class TestSmartContractAuditorIntegration:
    def test_deploy_and_analyze_through_consensus(self, mock_validators):
        ...
        receipt = contract.analyze_contract.transact(
            args=["Token", SAMPLE_LANGUAGE, SAMPLE_CODE.strip()],
            account=sender,
        )
        ...
        assert record["title"] == "Token"
        assert record["language"] == SAMPLE_LANGUAGE
